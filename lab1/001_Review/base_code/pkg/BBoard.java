...
public class BBoard {		// This is your main file that connects all classes.
	...
	public void loadUsers(String inputFile) throws FileNotFoundException {
		...
		bruhser = sc.nextLine();
		loginInfo.add(new User(bruhser.substring(0,bruhser.indexOf(" ")), bruhser.substring(bruhser.indexOf(" ")+1)));
	...
	public void login(){
		...
		for(int i = 0; i < loginInfo.size(); i++){
			if(loginInfo.get(i).check(user, pswrd)){
				logged = i;
				return;
			}
		}
	...
	public void run(){
		login();
		System.out.println("Welcome back " + loginInfo.get(logged).getUsername());
	}
